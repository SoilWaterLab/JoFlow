...
#1. Set Model Inputs
#Simulation Settings
outpath = 'C:/Isotope/CaseStudy_OH/output/'
...
R_Soil_Init = 0.00199 #Initial R for unsaturated zone storage
...
#1 - no mixing (macro pore concept, Todd's idea)
#2 - complete mixing (Chelsea's idea)
...
del_precip_obs = read.csv('C:/Isotope/CaseStudy_OH/Precip_Isotopes.txt')
R_precip_obs = (del_precip_obs$del_p/1000 + 1)*standard
#Reads in NCDC data with PRCP, TMIN, TMAX
precip= read.csv('C:/Isotope/CaseStudy_OH/NCDC_MetData_Station1.csv') #precip input is in inches
...
for(i in (2):length(deltaP))
{
  ...
  #Assumes even distribution of 18O stored in the soil matrix
