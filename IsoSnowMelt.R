IsoSnowMelt<-function(Date, precip_mm, R_precip_O, R_precip_H, Tmax_C, Tmin_C, lat_deg, slope=0, aspect=0, tempHt=1, windHt=2, groundAlbedo=0.25, 
  ...
  Snow_Heavy_O<-rep(0,length(precip_mm))			## 	m
  Snow_Heavy_H<-rep(0,length(precip_mm))			## 	m
  R_Snow_Heavy_O<-rep(0,length(precip_mm))
  R_Snow_Heavy_H<-rep(0,length(precip_mm))
  ...
  for (i in 2:length(precip_m)){
    ...
    #track isotopic composition of snow
    if(SnowWaterEq[i] > 0)
      {Snow_Heavy_O[i] = R_Snow_Heavy_O[i-1]*(SnowWaterEq[i-1] - SnowMelt[i]) + NewSnowWatEq[i]*R_precip_O[i]
        R_Snow_Heavy_O[i] = Snow_Heavy_O[i]/SnowWaterEq[i]
        Snow_Heavy_H[i] = R_Snow_Heavy_H[i-1]*(SnowWaterEq[i-1] - SnowMelt[i]) + NewSnowWatEq[i]*R_precip_H[i]
        R_Snow_Heavy_H[i] = Snow_Heavy_H[i]/SnowWaterEq[i]}
    
    if (SnowWaterEq[i] <= 0)
      {Snow_Heavy_O[i] = 0
        Snow_Heavy_H[i] = 0
        R_Snow_Heavy_O[i] = R_Snow_Heavy_O[i-1]
        R_Snow_Heavy_H[i] = R_Snow_Heavy_H[i-1]}
  }
  Results<-data.frame(Date, Tmax_C, Tmin_C, R_m*1000, NewSnowWatEq*1000,SnowMelt*1000, NewSnow, SnowDepth, R_Snow_Heavy_O, R_Snow_Heavy_H)
  colnames(Results)<-c("Date", "MaxT_C", "MinT_C", "Rain_mm", "SnowfallWatEq_mm", "SnowMelt_mm", "NewSnow_m", "SnowDepth_m","R_O","R_H")
  return(Results)
}
